...
/**
 * Opik client singleton for email triage observability.
 * 
 * Configure via environment variables:
 * - OPIK_API_KEY: Your Opik API key
 * - OPIK_URL_OVERRIDE: Opik API URL (default: https://www.comet.com/opik/api)
 * - OPIK_PROJECT_NAME: Project name (default: maxwell-email-triage)
...
export function getOpik(): Opik {
  if (!opikClient) {
    opikClient = new Opik({
      projectName: process.env.OPIK_PROJECT_NAME || 'maxwell-email-triage',
